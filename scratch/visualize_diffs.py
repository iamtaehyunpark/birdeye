...
output_dir = Path("scratch/masks")
output_dir.mkdir(exist_ok=True, parents=True)

names = ["red_sedan", "blue_suv", "white_pickup"]
...
for i, contour in enumerate(valid_contours[:3]):
    ...
    # Save the images
    cv2.imwrite(str(output_dir / f"{names[i]}_crop.png"), crop)
    cv2.imwrite(str(output_dir / f"{names[i]}_mask_thresh.png"), mask_thresh)
    cv2.imwrite(str(output_dir / f"{names[i]}_mask_solid.png"), mask_solid)
    cv2.imwrite(str(output_dir / f"{names[i]}_diff.png"), cv2.absdiff(mask_thresh, mask_solid))
    print(f"Saved visualization for {names[i]}")
